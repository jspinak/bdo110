...
public class ProcessingTransitions {
    ...
    public boolean toMainScreen() {
        log.info("Navigating from Processing to MainScreen");
    ...
    public boolean verifyArrival() {
        log.info("Verifying arrival at Processing");
        ...
        boolean found = action.find(processingState.getProcessingWindow(), findOptions).isSuccess();

        if (found) {
            log.info("Successfully arrived at Processing");
        } else {
            log.error("Failed to verify arrival at Processing");
        }
